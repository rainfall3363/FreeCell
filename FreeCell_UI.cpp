...
void Shell::printMessage(string status) {
    vector<string> ordinal = {"1st_Line", "2nd_Line", "3rd_Line", "4th_Line", "5th_Line", "6th_Line", "7th_Line", "8th_Line", "FreeCell", "HomeCell"};
    ...
    if (!status.compare("d")) {
        ...
        if (amount > 1) {
            cout << amount << " cards moved " << ordinal[from] << " to " << ordinal[to] << "\n"; 
        }
        else {
            cout << amount << " card moved " << ordinal[from] << " to " << ordinal[to] << "\n"; 
        }
    }
    else if (!status.compare("ud")) {
        cout << "Undo completed\n"; 
    }
    else if (!status.compare("nm")) {
        cout << "There are no moves left\n";
    }
    else if (!status.compare("um")) {
        cout << "!Warning: Wrong move!\n"; 
    }
    else if (!status.compare("uc")) {
        cout << "!Warning: Unvalid command!\n";
    }
